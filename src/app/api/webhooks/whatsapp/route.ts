import { whatsappAppSecret, whatsappVerifyToken } from "@/lib/whatsapp/config";
...
/**
 * Match the Resend webhook cap. The body is read with `request.text()` BEFORE
 * any signature check so the HMAC is computed over the exact raw bytes Meta
 * signed — never `request.json()` first.
 */
