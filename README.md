...
`bun run setup` starts local Supabase (OrbStack / any Docker Engine), writes `.env.local`, migrates, and seeds `owner@local.test` / `local-dev-password`. Then `bun run dev` and sign in with that email. Worktrees share that Docker stack and get their own Next port. `bun run doctor` checks the stack.
